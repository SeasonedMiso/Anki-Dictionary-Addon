...
PyObject* _Nullable PyObjCMetaClass_TryResolveSelector(PyObject* base, PyObject* name,
                                                       SEL sel)
{
    PyObjC_Assert(PyObjCClass_Check(base) || PyObjCMetaClass_Check(base)
                      || base == (PyObject*)&PyObjCMetaClass_Type,
                  NULL);
    Class     cls;
    Method    m;
    ...
    if (m) {
        ...
        PyObject* result = PyObjCSelector_NewNative(
            cls, sel, (const char* _Nonnull)method_getTypeEncoding(m), 1);
        if (result == NULL) { // LCOV_BR_EXCL_LINE
            return NULL;      // LCOV_EXCL_LINE
        }
    ...
    for (i = 0; i < n; i++) {
        ...
        if (PyObjCClass_Check(base)) {
            ...
            if (m) {
                ...
                /* Create (unbound) selector */
                PyObject* result = PyObjCSelector_NewNative(
                    cls, sel, (char* _Nonnull)method_getTypeEncoding(m), 0);
                if (result == NULL) { // LCOV_BR_EXCL_LINE
                    Py_CLEAR(mro);
                    return NULL; // LCOV_EXCL_LINE
                }
